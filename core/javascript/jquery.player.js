		...
		var methods = {
				...
				if(self.config.buttons.toggle){	// If the toggle button is enabled
					var $toggle = self.createButton('play', 'Play').attr({'aria-live':'assertive'}).click(function(){
						if($(this).hasClass('play')){
							$(this).removeClass('play').addClass('pause').attr({ 'id':'pause-'+self.config.id}).text('Pause');
							self.play();
						}else{
							$(this).removeClass('pause').addClass('play').attr({ 'id':'play-'+self.config.id}).text('Play');
							self.pause();
						}
					});
					buttons.push($toggle);
			...
			*---------------------------------------------------------*/
			getSlider : function(){
				...
				$sliderBar.find('a.ui-slider-handle').attr({'role':'slider','aria-valuemin':'0','aria-valuemax':'100','aria-valuenow':'0', 'aria-valuetext':'0 percent'});
